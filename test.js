	...
	$("#end").on("click", function(){ //podsumowanie
		...
		$("#restart").on("click", function(){
			...
			$("#totalScore").show(); //pokazuje łączny wynik
											...
											/*KONIEC MATMY ZACZYNAMY POLSKI!!!!*/	
											
		//quiz3 dyktando
		
$("#Quiz3").on("click", function(){
	dyktando = [dykPyt1, dykPyt2,dykPyt3,dykPyt4,dykPyt5];//nadaje wartość zmiennej
	dykOdp = [poprawne1,poprawne2,poprawne3, poprawne4,poprawne5]; //nadaje wartość odpowiedziom
	$(".welcome").hide(); 
	$(".container").slideDown(1000);
	$("p").html(dyktando[h][j]); //ustawia odpowiednią wartość pytania [0][0]
	$('.inputs').first().focus(); //ustawia skupienie na pierwszym input
	$('#getAnswer').hide(); //chowa pozostałości po pierwszym quizie
	$('#submitAnswer').hide(); //chowa pozostałości po pierwszym quizie
	$('#next').hide(); //chowa pozostałości po pierwszym quizie
	$('#nextDyk').show(); //pokazuje możliwośc przeskoczenia do następnego pytania (nieaktywne)
	$('#questionsQuantity').html(iloscPytDyk); //ustawia liczbę pytań
	$("#points").text(liczbaPunktow); //ustawia liczbę punktów
	$("#questionNumber").text(iloscWpisDyk); //ustawia nr pytania a w zasadzie wpisu
	animowanyPasek(iloscWpisDyk,4); //ustawia pasek na pozycji początkowej
});
...
$("#Quiz4").on("click", function(){
	...
	animowanyPasek(iloscWpisDyk,4);
		...
		//usuwa disabled z kolejnego i rozpoczyna grę nie dało się tego zrobić jednym focus out
...
$('p').on("focusout", '.inputs', function(){
	$(this).each(function(){
		if($(this).val()===dykOdp[h][j]){																				 //blokuje już zrobione
		$(this).css('background-color', '#59B23F').attr("disabled","disabled").next(".inputs").focus()//nadaje kolor zielony 
		liczbaPunktow++;//suma punktów																				//przeskakuje do następnego
		iloscWpisDyk++;//czy źle czy dobrze, suma wszystkich przekliknięć
		}
		//jeżeli odpowiedz zła
		else {
		$(this).css('background-color','#B22518').attr("disabled","disabled").next(".inputs").focus();//czerwony
		iloscWpisDyk++;
		}
		//kolejna odpowiedź
		j++ //zwiększa indeks
		...
		//zmiana punktów
	$("#points").text(liczbaPunktow);//zmienia punkty
	$("#questionNumber").text(iloscWpisDyk);//zmienia ilosc wpisów
	animowanyPasek(iloscWpisDyk,4); //rusza paskiem
});
...
$("#endDyk").on("click", function(){
	$(this).attr("disabled","disabled").hide();//chowa to
	var wynikDyk = liczbaPunktow/iloscWpisDyk*100 //oblicza wynik
	$("#total").text(lacznyWynik(liczbaPunktow));//przekazuje wynik
	$("#maximum").text(max(iloscWpisDyk)); //przekazuje max możliwy
	$("#restartDyk").show(); //pkazuje restart
	$("p").fadeOut(); //ukrywa pytanie
	podsumowanie(wynikDyk,imie);//wywołuje podsumowanie
});
...
$("#restartDyk").on("click", function(){
	...
	dyktando =[]; //czyści
	dykOdp = []; //czyści
	liczbaPunktow=0; //zeruje
	iloscWpisDyk=0; //zeruje
	$(".welcome").show(); 
	$(".container").hide();
	$("#thankYou").hide();
	$("#summary").hide();
...
$("form").on("click", "input[type='radio']:checked", function(){
	licznik++;//liczy liczbę pytań
	if(licznik==10 && $(this).val()!=''){ //poprawna ostatnia odpowiedź
		liczbaPunktow++; 
		$("#points").text(liczbaPunktow);
		$("#endRadio").prop("disabled",false);
			}
	else if(licznik==10){ //niepoprawna ostatnia odpowiedź
		$("#endRadio").prop("disabled",false); 
	}
	else if($(this).val()!=''){ //poprawne pozostałe odpowiedzi
		liczbaPunktow++;
		$("#points").text(liczbaPunktow);
		...
		$("input[type='radio'").attr("disabled","disabled"); //niepoprawne odpowiedzi
		$("#nextRadio").prop("disabled",false);
	}
});
...
$("#nextRadio").on("click", function(){
	...
	$("input[type='radio']").each(function(){ //wstawienie prop i treści
		$(this).prop('value',radioVal[k][g]);
...
$("#changePlayer").on("click",function(){
	$(this).attr("disabled","disabled");//blokuje ten przycisk
	name.removeAttr("disabled").focus().val('');//przywraca możliwość wprowadzenia
	$("#totalScore").hide(); //chowa całkowity wynik
	$(".startQuiz").attr("disabled","disabled");//blokuje wybór quizu
	wyniki.push(wynikProcent(nowaSuma,sumaMax));//tworzy tablicę z dotychczasowymi wynikami
	imiona.push(imie);//tworzy tablicę z graczami, którzy brali udział w grze
	var indeksy = wyniki.map(function(element, index){
	return index; //zwraca nową tablicę z indeksami 
	});
	indeksy.sort(function (b,a) {return wyniki[a] - wyniki[b]; //sortuje indeksy
	});
	posortowane = ''
	for(var i=0; i < wyniki.length; i++){
	posortowane+="<li>" + imiona[indeksy[i]] + ": " + wyniki[indeksy[i]] +"%</li>"
	}//tworzy listę z imionami i pasującymi do nich wynikami od najlepszego wyniku
	$("ol").fadeIn().html("Ranking:" + posortowane);//umieszcza listę w ol
});
		//progres bar
...
function podsumowanie(wynik,imie) {
	if(wynik < 40) {	
	$("#summary").fadeIn().html(imie + " twój wynik to " + Math.ceil(wynik) + "%. Następnym razem postaraj sibardziej").css("color","red");
