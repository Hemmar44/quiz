...
//ekran początkowy
	...
	$("#end").on("click", function(){
		...
		$("#changePlayer").on("click",function(){
			...
			name.focus().val('');
			$("ol").slideDown(800).append("<li>" + wynikProcent(nowaSuma,sumaMax) +  "</li>"); //"-" + imie +
			//alert(nowaSuma);
			//alert(sumaMax);
			...
			posortowane = '';
			$("li").each(function(){
				wyniki.push($(this).text());
			});
			alert("przed sortem " + wyniki);
			sortuj(wyniki);
			alert("po sorcie " + wyniki);
			$.each(wyniki, function(key,element){
			posortowane+="<li>" +element+ "</li>"
			});
			$("ol").html(posortowane);
