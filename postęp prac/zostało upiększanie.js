...
//ekran początkowy
	...
	$("#end").on("click", function(){
		...
		//lacznyWynik(iloscGier,liczbaPunktow);
		$("p").fadeOut();
		$("#total").text(lacznyWynik(liczbaPunktow));
		$("#maximum").text(max(liczbaPytan));
		//alert("lacznyWynik1 " + lacznyWynik(iloscGier, liczbaPunktow));
		podanaOdpowiedz.hide();
		$("#restart").show();
		//iloscGier = 1;
		//alert("iloscGier " + iloscGier);
		//alert("lacznyWynik2 " + lacznyWynik(iloscGier, liczbaPunktow));
		...
		$("#changePlayer").on("click",function(){
			...
			name.focus().val('');
			//$("ol").show().append("<li>" + imie + "</li><span class='playerScore'>" + wynikProcent(nowaSuma,sumaMax)  + "</span>"); //"-" + imie +
			//alert(nowaSuma);
			//alert(sumaMax);
			//alert(wynikProcent(nowaSuma,sumaMax));
			//imiona = [];
			//wyniki = [];
			alert(wynikProcent(nowaSuma,sumaMax));
			alert(imie);
			...
			//sortuj(wyniki);
			//alert("po sorcie " + wyniki);
			//$.each(wyniki, function(i,element){
			//posortowane+="<li>" + i + ": " +element+ "</li>"
			//});
			//$("ol").html(imie + wynikProcent(nowaSuma,sumaMax));
	...
	//podsumowanie 
		...
		var lacznyWynik = function(punkty){
			nowaSuma+=punkty;
			return nowaSuma;
		}
		
		var max = function(maxpunty) {
			sumaMax+=maxpunty;
			return sumaMax;
		}
		var wynikProcent = function(nowaSuma,sumaMax){
			if(sumaMax!=0){
